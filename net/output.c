...
extern union Nsipc nsipcbuf[16];
...
output(envid_t ns_envid)
{
    ...
    while (true) {
        ...
        if (req_type == NSREQ_OUTPUT){
            ...
            send_packet(nsipcbuf[0].pkt.jp_data, nsipcbuf[0].pkt.jp_len, true);
            sys_page_unmap(curenv->env_id, &nsipcbuf[0].pkt);
        }
            ...
            int num_of_pkts = ipc_recv_multi(&nsipcbuf, &perm);
            if (num_of_pkts <= 0){
